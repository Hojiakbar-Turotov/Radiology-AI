...
function mapDoctorByRoom(roomStr, docsAuth, doctorKeys) {
  ...
  for (const key of doctorKeys) {
    ...
    if (!d) continue;
    if (d.shortName && low.includes(d.shortName.toLowerCase())) return d;
    if (d.doctorName && low.includes(d.doctorName.toLowerCase())) return d;
    const parts = (d.doctorName || '').toLowerCase().split(' ');
...
function processAnalytics(rawRecords, startDate, endDate) {
  ...
  rawRecords.forEach((p, index) => {
    ...
    const targetDoctor = docAccepting || docRoom;
