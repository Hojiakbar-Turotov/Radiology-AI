...
class SmartScheduler {
  ...
  calculateExamDurationForSlot(analysis, onShiftLaborants) {
    ...
    // Foydalanuvchi talabi: Agar bir bemorga 2 yoki undan ortiq tekshiruv qo'yilsa:
    // MRT tekshiruvlarida organlar uchun tekshiruv vaqtlari qo'shilsin
    // MSKT tekshiruvida eng katta vaqt sarflanadigan tekshiruv vaqti umumiy vaqt deb olinsin
    if (analysis.serviceCount > 1) {
      const ruleText = analysis.deviceType === "MSKT"
        ? `${analysis.serviceCount} ta MSKT tekshiruvi: Eng katta vaqt sarflanadigan tekshiruv vaqti tanlandi (${adminStandardTime} daqiqa)`
        : `${analysis.serviceCount} ta organ MRT tekshiruvi: Barcha organlar tekshiruv vaqtlari to'liq qo'shildi (${adminStandardTime} daqiqa)`;
      return {
        duration: adminStandardTime,
        ruleApplied: "multi_service_rule",
        ruleDescription: ruleText,
        activeLaborantNames: (onShiftLaborants || []).map(l => l.name || l.login)
      };
    }
    ...
    const laborantDurations = onShiftLaborants.map(lab => {
      ...
      // 1. Agar laborant ushbu aniq tekshiruv kodi (masalan, "R157") bo'yicha shaxsiy vaqt kiritgan bo'lsa
      if (prefs[serviceCode]) {
        customTime = parseInt(prefs[serviceCode], 10);
      }
      // 2. Aks holda, Admin tomonidan katalogda belgilangan standart vaqt asos qilib olinadi!
      // Foydalanuvchi talabi: "30 minutli tekshiruv bo'lsa 30 minut vaqt band qilinsin."
      else if (adminStandardTime && adminStandardTime >= 10) {
        customTime = adminStandardTime;
      }
      // 3. Fallback: Kategoriya bo'yicha
      ...
      const finalDuration = (customTime && customTime >= 5) ? customTime : (adminStandardTime || 25);
      return {
        laborantName: lab.name || lab.login,
        duration: finalDuration,
        hasCustom: Boolean(prefs[serviceCode])
      };
    });
