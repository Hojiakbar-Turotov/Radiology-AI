  ...
  analyzeServices(servicesList) {
    ...
    servicesList.forEach((srv, idx) => {
      ...
      if (catItem) {
        ...
        totalStandardDuration += idx === 0 ? itemDuration : Math.round(itemDuration * 0.75);
        if (!primaryPrep && catItem.preparation) primaryPrep = catItem.preparation;
        if (!primaryContra && catItem.contraindications) primaryContra = catItem.contraindications;
      } else {
    ...
    return {
      ...
      preparation: primaryPrep,
      contraindications: primaryContra
    };
  }
