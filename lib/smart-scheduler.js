...
class SmartScheduler {
  ...
  upsertService(serviceData, requesterUser) {
    ...
    const name = serviceData.name ? String(serviceData.name).trim() : (existing.name || "");
    if (!name) throw new Error("Tekshiruv nomi kiritilishi shart");
    ...
    const item = {
      ...
      preparation: serviceData.preparation !== undefined ? String(serviceData.preparation).trim() : (existing.preparation || ""),
      contraindications: serviceData.contraindications !== undefined ? String(serviceData.contraindications).trim() : (existing.contraindications || ""),
      consentQuestionIds: Array.isArray(serviceData.consentQuestionIds) ? serviceData.consentQuestionIds : (existing.consentQuestionIds || []),
   ...
   * Oylik/yillik taqvimdagi maxsus kunlar (customDates) va standart kunlar asosida tekshiradi.
   */
  getLaborantsOnShift(targetDate, timeStr, deviceId = null) {
    ...
    for (const lab of laborants) {
      ...
      // 1. Shaxsiy oylik/yillik taqvim (customDates) tekshiruvi
      let isWorkDay = null;
      let dayStart = ws.start || "08:00";
      let dayEnd = ws.end || "17:00";
      let dayLunchStart = ws.lunchStart || "12:00";
      let dayLunchEnd = ws.lunchEnd || "13:00";

      if (ws.customDates && ws.customDates[dateStr] !== undefined) {
        const cVal = ws.customDates[dateStr];
        if (typeof cVal === 'string') {
          if (cVal === 'off') isWorkDay = false;
          else if (cVal === 'work' || cVal === 'duty') isWorkDay = true;
        } else if (typeof cVal === 'object' && cVal !== null) {
          if (cVal.type === 'off' || cVal.isWorkDay === false) {
            isWorkDay = false;
          } else {
            isWorkDay = true;
            if (cVal.start) dayStart = cVal.start;
            if (cVal.end) dayEnd = cVal.end;
            if (cVal.lunchStart) dayLunchStart = cVal.lunchStart;
            if (cVal.lunchEnd) dayLunchEnd = cVal.lunchEnd;
          }
        }
      }

      // Agar taqvimda aniq belgilanmagan bo'lsa -> haftalik standart kunlar (ws.days) bo'yicha
      if (isWorkDay === null) {
        const days = ws.days || ["Dush", "Sesh", "Chor", "Pay", "Jum", "Shan"];
        isWorkDay = days.includes(shortDayName);
      }

      if (!isWorkDay) continue;

      // 2. Ish soati oralig'idami?
      if (timeStr < dayStart || timeStr >= dayEnd) continue;

      // 3. Tushlik vaqti emasmi?
      if (timeStr >= dayLunchStart && timeStr < dayLunchEnd) continue;
  ...
  /**
   * Muayyan sanada apparat uchun birorta ham laborant ishda (navbatchilikda) bormi-yo'qligini tekshirish
   */
  hasAnyLaborantOnDuty(targetDate, deviceId = null) {
    const allUsers = this.getStaffUsers();
    const dayIndex = targetDate.getDay();
    const shortDayName = DAY_NAMES[dayIndex];
    ...
    for (const lab of laborants) {
      if (deviceId && lab.room) {
        const roomLower = lab.room.toLowerCase();
        if (deviceId.includes('mrt1') && (roomLower.includes('mrt 2') || roomLower.includes('mskt'))) continue;
        if (deviceId.includes('mrt2') && (roomLower.includes('mrt 1') || roomLower.includes('mskt'))) continue;
        if (deviceId.includes('mskt') && roomLower.includes('mrt')) continue;
      }
      ...
      if (ws.customDates && ws.customDates[dateStr] !== undefined) {
        ...
        if (typeof cVal === 'string') {
          ...
          isWorkDay = (cVal.type !== 'off' && cVal.isWorkDay !== false);
        }
      }
  ...
  analyzeServices(servicesList) {
    ...
    const names = [];
    const allPrepItems = [];
    const allContraItems = [];
    const parsedDurations = [];
    const consentQuestionIds = [];

    const parsedServicesDetails = [];
    ...
    servicesList.forEach((srv, idx) => {
      ...
      const srvName = srv.name || srv.serviceName || (catItem ? catItem.name : "");
      if (srvName) names.push(srvName);

      const itemDuration = srv.duration ? parseInt(srv.duration, 10) : (catItem ? catItem.duration : 25);
      parsedDurations.push(itemDuration);

      let itemPrep = srv.preparation || (catItem ? catItem.preparation : "");
      let itemContra = srv.contraindications || (catItem ? catItem.contraindications : "");

      parsedServicesDetails.push({
        code: code,
        name: srvName,
        defaultDuration: itemDuration,
        preparation: itemPrep,
        contraindications: itemContra
      });

      if (catItem) {
        if (catItem.isContrast) isContrast = true;
        if (catItem.type === "MSKT") deviceType = "MSKT";
        if (catItem.preparation) allPrepItems.push(catItem.preparation);
        if (catItem.contraindications) allContraItems.push(catItem.contraindications);
        if (Array.isArray(catItem.consentQuestionIds)) {
          catItem.consentQuestionIds.forEach(id => {
            if (!consentQuestionIds.includes(id)) consentQuestionIds.push(id);
          });
        }
      } else {
        ...
        if (srv.preparation) allPrepItems.push(srv.preparation);
        if (srv.contraindications) allContraItems.push(srv.contraindications);
      }

      if (Array.isArray(srv.consentQuestionIds)) {
        srv.consentQuestionIds.forEach(id => {
          if (!consentQuestionIds.includes(id)) consentQuestionIds.push(id);
        });
      }
    });
    ...
    // 1. MRT tekshiruvlarida organlar uchun tekshiruv vaqtlari qo'shilsin (T_1 + T_2 + ...)
    // 2. MSKT tekshiruvida 2 yoki undan ortiq tekshiruvlar ichidan eng katta vaqt sarflanadigan tekshiruv vaqti umumiy vaqt deb qabul qilib olinsin (max(T_1, T_2, ...))
    let totalStandardDuration = 0;
    if (deviceType === "MSKT") {
      totalStandardDuration = parsedDurations.length > 0 ? Math.max(...parsedDurations) : 20;
    } else {
      // MRT tekshiruvi: barcha organlarning vaqtlari qo'shiladi
      totalStandardDuration = parsedDurations.reduce((sum, d) => sum + d, 0);
    }

    if (totalStandardDuration < 15) totalStandardDuration = 15;

    // Unikal tayyorgarlik punktlari (takrorlanishlarsiz)
    const uniquePrep = this.deduplicateGuidelines(allPrepItems, isContrast);

    // Unikal qarshi ko'rsatma punktlari (takrorlanishlarsiz)
    const uniqueContra = this.deduplicateGuidelines(allContraItems, isContrast);

    const primaryName = names.join(" + ") || "MRT Tekshiruvi";
    ...
    return {
      ...
      preparation: uniquePrep,
      contraindications: uniqueContra,
      consentQuestionIds,
      services: parsedServicesDetails
    };
  }

  /**
   * Bir nechta tekshiruvlar tayyorgarligi va qarshi ko'rsatmalarini takrorlanishsiz birlashtirish
   */
  deduplicateGuidelines(itemsSources, isContrast = false) {
    if (!Array.isArray(itemsSources)) itemsSources = [itemsSources];
    const seen = new Set();
    const result = [];

    const allCombined = itemsSources.filter(Boolean).join('\n').toLowerCase();
    const contrastPresent = isContrast || allCombined.includes('kontrast') || allCombined.includes('kreatinin') || allCombined.includes('och qoringa');

    itemsSources.forEach(src => {
      if (!src) return;
      ...
      lines.forEach(rawLine => {
        ...
        // Agar kontrast mavjud bo'lsa, "parhez/och qolish talab etilmaydi" bandini tushirib qoldirish
        if (contrastPresent && (line.toLowerCase().includes('och qolish talab etilmaydi') || line.toLowerCase().includes('parhez talab etilmaydi'))) {
          return;
        }
  ...
  calculateExamDurationForSlot(analysis, onShiftLaborants) {
    ...
    const services = (Array.isArray(analysis.services) && analysis.services.length > 0)
      ? analysis.services
      : [{ code: analysis.serviceCode, defaultDuration: adminStandardTime, name: analysis.primaryName }];

    const isContrast = analysis.isContrast;
    const deviceType = analysis.deviceType; // "MRT" | "MSKT"

    // Har bir smenadagi laborant uchun ushbu bemor tekshiruvlarining hisoblangan umumiy vaqti
    const laborantDurations = onShiftLaborants.map(lab => {
      ...
      const perServiceDurations = services.map(srv => {
        let srvTime = null;
        if (srv.code && prefs[srv.code]) {
          srvTime = parseInt(prefs[srv.code], 10);
          hasAnyCustom = true;
        } else if (deviceType === "MRT") {
          if (isContrast && prefs["MRT_KONTRAST"]) srvTime = parseInt(prefs["MRT_KONTRAST"], 10);
          else if (!isContrast && prefs["MRT_ODDIY"]) srvTime = parseInt(prefs["MRT_ODDIY"], 10);
          else if (prefs["MRT"]) srvTime = parseInt(prefs["MRT"], 10);
        } else if (deviceType === "MSKT") {
          if (isContrast && prefs["MSKT_KONTRAST"]) srvTime = parseInt(prefs["MSKT_KONTRAST"], 10);
          else if (!isContrast && prefs["MSKT_ODDIY"]) srvTime = parseInt(prefs["MSKT_ODDIY"], 10);
          else if (prefs["MSKT"]) srvTime = parseInt(prefs["MSKT"], 10);
        }

        if (!srvTime || isNaN(srvTime) || srvTime < 5) {
          srvTime = srv.defaultDuration || (deviceType === "MSKT" ? 20 : 25);
        }
        return srvTime;
      });

      let totalLabDuration = 0;
      if (deviceType === "MSKT") {
        totalLabDuration = Math.max(...perServiceDurations);
      } else {
        // MRT tekshiruvi: barcha organlar vaqtlari qo'shiladi
        totalLabDuration = perServiceDurations.reduce((sum, d) => sum + d, 0);
      }

      if (totalLabDuration < 15) totalLabDuration = 15;

      return {
        laborantName: lab.name || lab.login,
        duration: totalLabDuration,
        perServiceDurations,
        hasCustom: hasAnyCustom
      };
    });
    ...
    if (laborantDurations.length === 1) {
      ...
      let ruleDesc = `Laborant ${single.laborantName} (${single.duration} daqiqa)`;
      if (services.length > 1) {
        ruleDesc = deviceType === "MSKT"
          ? `Laborant ${single.laborantName}: ${services.length} ta MSKT (eng kattasi: ${single.duration} daq)`
          : `Laborant ${single.laborantName}: ${services.length} ta organ MRT qo'shildi (${single.duration} daq)`;
      }
  ...
  findNextAvailableSlot(patientInput) {
    ...
    for (let dayOffset = 0; dayOffset < 30; dayOffset++) {
      ...
      // Agar laborantlar o'z ish kunlarini belgilagan bo'lsa va bu kunda ushbu apparatda navbatchi laborant bo'lmasa:
      // (Faqat foydalanuvchi majburiy sana ko'rsatmagan bo'lsa, aqlli tizim laborant ishlaydigan kunni tanlaydi!)
      if (!patientInput.scheduledDate && !this.hasAnyLaborantOnDuty(candidateDate, chosenDeviceId)) {
        continue;
      }
      ...
      // Agar foydalanuvchi aniq sana tanlagan bo'lsa, faqat shu sanani tekshirish
      if (patientInput.scheduledDate && dateStr !== patientInput.scheduledDate) {
        continue;
      }
      ...
      // Agar foydalanuvchi aniq soat ko'rsatgan bo'lsa (masalan, 13:00)
      if (patientInput.scheduledTime) {
        const [rh, rm] = patientInput.scheduledTime.split(':').map(Number);
        if (!isNaN(rh) && !isNaN(rm)) {
          const reqMin = rh * 60 + rm;
          if (reqMin > (startHour * 60 + startMinute)) {
            startHour = Math.floor(reqMin / 60);
            startMinute = reqMin % 60;
          }
        }
      }
      ...
      while (currentCheckMinute < clinicClosingMinute) {
        ...
        // 3. Mavjud bemorlar bilan to'qnashuvni (Overlap) tekshirish: Qat'iy bitta vaqtga 2 ta bemor qo'yilmasin!
        ...
        for (const existing of dayQueue) {
          ...
          if (existing.scheduledTime) {
            const [sh, sm] = existing.scheduledTime.split(':').map(Number);
            if (!isNaN(sh) && !isNaN(sm)) {
              exStartMin = sh * 60 + sm;
              if (existing.finishTime) {
                const [fh, fm] = existing.finishTime.split(':').map(Number);
                if (!isNaN(fh) && !isNaN(fm)) exFinishMin = fh * 60 + fm;
              }
              if (exFinishMin <= exStartMin) {
                exFinishMin = exStartMin + (existing.estimatedDurationMinutes || 25);
              }
            }
          }

          if (exStartMin === -1 && existing.estimatedStartTime) {
            const dStart = new Date(existing.estimatedStartTime);
            if (!isNaN(dStart.getTime())) {
              exStartMin = dStart.getHours() * 60 + dStart.getMinutes();
              if (existing.estimatedFinishTime) {
                const dFinish = new Date(existing.estimatedFinishTime);
                if (!isNaN(dFinish.getTime())) {
                  exFinishMin = dFinish.getHours() * 60 + dFinish.getMinutes();
                }
              }
              if (exFinishMin <= exStartMin) {
                exFinishMin = exStartMin + (existing.estimatedDurationMinutes || 25);
          ...
          if (exStartMin === -1 || exFinishMin === -1) continue;

          // Oraliqlar to'qnashuvi: [currentCheckMinute, finishMinute] va [exStartMin, exFinishMin]
          // Agar yangi bemor vaqti mavjud bemor vaqtiga to'g'ri kelsa
          if (currentCheckMinute < exFinishMin && finishMinute > exStartMin) {
            hasConflict = true;
            // To'qnashuv bo'lsa, tekshirish vaqtini mavjud bemor tugaydigan vaqtga siljitish
            currentCheckMinute = exFinishMin;
            break;
          }
        }
        ...
        if (!hasConflict) {
          ...
          return {
            ...
            estimatedStartTime: `${dateStr} ${timeStr}`,
            estimatedFinishTime: `${dateStr} ${finishTimeStr}`,
            estimatedStartTimeISO: startDateTime.toISOString(),
            estimatedFinishTimeISO: finishDateTime.toISOString(),
