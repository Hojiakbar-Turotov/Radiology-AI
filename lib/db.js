...
const DEFAULT_CONSENT_QUESTIONS = [
  ...
  },
  {
    ...
    riskLevel: "warning",
    dangerAnswer: "yes",
    description: "Homiladorlikning 1-trimestrida (birinchi 12 hafta) faqat hayotiy ko'rsatma bo'yicha o'tkaziladi.",
    required: true
  },
  {
    id: "cq_mrt_tattoo",
  ...
  {
    id: "cq_mskt_pregnancy",
    category: "MSKT",
    order: 9,
    text: "Homiladorlik holatidamisiz yoki homiladorlik ehtimoli bormi?",
    riskLevel: "danger",
    dangerAnswer: "yes",
    description: "Rentgen nurlanishi homilaga salbiy ta'sir ko'rsatishi mumkin. Homiladorlikda MSKT qat'iyan taqiqlanadi.",
    required: true
  },
  {
    id: "cq_mskt_barium",
...
class LocalDatabase {
  ...
  getConsentQuestions(category) {
    if (!category || category === "ALL") return this.consentQuestions;
    return this.consentQuestions.filter(q => q.category === category || q.category === "ALL");
  }
  ...
  saveConsentQuestion(data) {
    ...
    if (item) {
      item.text = String(data.text).trim();
