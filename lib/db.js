...
class LocalDatabase {
  ...
  addPatient(patientData) {
    ...
    const newPatient = {
      ...
      sampleNumber: String(patientData.sampleNumber || patientData.sampleNo || patientData.sample || '').trim(),
      ...
      presenceStatus: patientData.presenceStatus || 'waiting_room', // 'waiting_room' (Kutish zalida) yoki 'not_arrived' (Hali kelmagan)
  ...
  updatePatientPresence(id, presenceStatus) {
    const patient = this.queue.find(p => p.id === id);
    if (!patient) return null;
    patient.presenceStatus = presenceStatus; // 'waiting_room' yoki 'not_arrived'
  ...
  updatePatientStatus(id, newStatus, extraData = {}) {
    ...
    // Agar xonaga yoki tayyorgarlikka chaqirilsa, avtomatik zalda deb hisoblanadi
    if (newStatus === 'calling' || newStatus === 'in_progress' || newStatus === 'preparing') {
      patient.presenceStatus = 'waiting_room';
    }
